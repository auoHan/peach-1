...
nginx_container_name=peach-nginx-1
db_container_name=db-for-peach
...
if [ "$(docker ps -aq -f name=^${DB_HOST}$)" ]; then
  ...
  docker run -d --name $DB_HOST \
            --network=network1 \
            -e POSTGRES_USER=peach \
            -e POSTGRES_DB=peach_production \
            -e POSTGRES_PASSWORD=$DB_PASSWORD \
            -e PGDATA=/var/lib/postgresql/data/pgdata \
            -v peach-data:/var/lib/postgresql/data \
            postgres:14
  echo '创建成功'
fi

title 'app: docker build'
docker build $root -t peach:$version
